# using Test, Hyperopt, ForwardDiff, Distributions
using Test, Hyperopt, Distributions
...
hardcode_dims = [Hyperopt.ContinuousDim(), Hyperopt.CategoricalDim(10), Hyperopt.UnorderedCategoricalDim(10)]
...
function gen_data(dim_type)
    local _dt
    if dim_type isa Hyperopt.CategoricalDim
        _dt = rand(1:categorical_level)
    elseif dim_type isa Hyperopt.UnorderedCategoricalDim
        _dt = unordered_objs[rand(1:length(unordered_objs))]
    elseif dim_type isa Hyperopt.ContinuousDim
...
function kde_gradient(multi_kde, x, delta=1e-10)
    x1 = [_kde.type isa Hyperopt.ContinuousDim ? _x-delta : 
                            (_kde.type isa Hyperopt.UnorderedCategoricalDim ? _x : _x-delta) 
                            for (_kde, _x) in zip(multi_kde.KDEs, x)]
    x2 = [_kde.type isa Hyperopt.ContinuousDim ? _x+delta : 
                            (_kde.type isa Hyperopt.UnorderedCategoricalDim ? _x : _x+delta) 
                            for (_kde, _x) in zip(multi_kde.KDEs, x)]
    (Hyperopt.pdf(multi_kde, x2) - Hyperopt.pdf(multi_kde, x1)) / delta
end
...
function kde_gradient_to_point(multi_kde, x, y, delta=1e-10)
    vec = [_kde.type isa Hyperopt.ContinuousDim ? _x-_y : 
                            (_kde.type isa Hyperopt.UnorderedCategoricalDim ? nothing : _x-_y) 
...
@testset "multi_kde" begin
    ...
    for bandwidth in hardcode_bws
        multi_kde = Hyperopt.MultivariateKDE(hardcode_dims, bandwidth, hardcode_data)
        _result = [Hyperopt.pdf(multi_kde, _data) for _data in hardcode_data_pred]
        push!(result, _result)
    end
    @test sum(result .≈ hardcode_expected_result) == length(hardcode_expected_result)
    # Variables needed for Sanity-check
    dim_types = [Hyperopt.ContinuousDim, Hyperopt.CategoricalDim, Hyperopt.UnorderedCategoricalDim]
    ...
    for num_dim in num_dims
        ...
        # Prepare Random MultivariateKDE
        _dims = dim_types[randint(num_dim, length(dim_types))]
        _dims = [_dim==Hyperopt.ContinuousDim ? _dim() : (_dim==Hyperopt.CategoricalDim ? _dim(categorical_level) : _dim(unordered_level)) for _dim in _dims]
        ...
        for (i, __dims) in zip(1:length(_dims), _dims)
            if __dims isa Hyperopt.UnorderedCategoricalDim
                candidates[i] = unordered_objs
            end
        end
        multi_kde2 = Hyperopt.MultivariateKDE(_dims, _bw2, dt, candidates)
        multi_kde3 = Hyperopt.MultivariateKDE(_dims, _bw3, dt, candidates)
        ...
        middle = [_dim isa Hyperopt.ContinuousDim ? (_max-_min) / 2 : 
                    (_dim isa Hyperopt.UnorderedCategoricalDim ? (multi_kde3.index_to_unordered[_kde][round(Int, (_max+_min)/2)]) : round(Int, (_max+_min)/2))
        ...
        for i in 1:num_s2_test
            point_out = [_dim isa Hyperopt.ContinuousDim ? rand([mins[i]-rand(Uniform(0, ranges[i])), maxs[i]+rand(Uniform(0, ranges[i]))]) : 
                            (_dim isa Hyperopt.UnorderedCategoricalDim ? rand([multi_kde3.index_to_unordered[_kde][1], multi_kde3.index_to_unordered[_kde][length(multi_kde3.index_to_unordered[_kde])]]) : 
